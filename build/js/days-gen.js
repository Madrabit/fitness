...
(function () {
	...
	// Delete Exercise with DnD

    //----------------- Dran n Drop

    let draggableEx =  document.querySelectorAll('.day_item');

    draggableEx.forEach( function (el){
        el.draggable = "true";
    })

    const draggbleListEx =  document.querySelector('.day');
    let draggedItemEx = null;


    const delDnDCard = (draggedItemEx) => {

        draggedItemEx.remove();
    ...
    let deletByDnd = () => {
        let dropeZone = document.querySelector('.wrapper');

        draggbleListEx.addEventListener('dragstart', function (e) {
            if (e.target.className === 'day_item ex') {
                draggedItemEx = e.target;
        ...
        dropeZone.addEventListener('drop', function (e) {
            ...
            if(e.target.className === 'content__main days' || e.target.className === 'search-ex__pic') {

                // exercises.push(newObj);
                delDnDCard(draggedItemEx);
            }
        ...
        draggbleListEx.addEventListener('dragend', function (e) {
            e.preventDefault();
